...
public class leWords extends BasicGame {
	...
	public void init(GameContainer container) throws SlickException {
		...
		Nfont = new Font("Arial", Font.PLAIN, 0);
		Nlistfont = new TrueTypeFont(Nfont, true);
		...
		oButton = new Field(510, 570, 0, 0, 'O');
		kButton = new Field(530, 570, 0, 0, 'K');

		tf = new TextField(container, Nlistfont, 350, 400, 400, 100);
		...
		// Has to be AFTER initField()! (Danger of being out of synch with the
		// server else!
	...
	public void update(GameContainer container, int delta)
		...
		if (!hasName) {
			...
			if (input.isMouseButtonDown(0)
					&& (oButton.update(mouseX, mouseY) || kButton.update(
							mouseX, mouseY)) && !tf.getText().equals("")) {
					...
					if (field[i][j].update(mouseX, mouseY)) {
						...
						if (field[i][j].selected == false) {
							...
							if (selected == null) {
								 ...
								 * selection.elementAt(selection.size()-1).
								 * neighbours.contains(selected))
								 */field[i][j].neighbours.contains(selected)) {
							/*
							 * thomas : bug occours ! you can only go back the
							 * LAST one in list, this is not the case
							 */
	...
	public void render(GameContainer container, Graphics g)
		...
		// bg.draw(0,0);
		if (!hasName) {
			tileStringPrint(">", 370, 400, 1);
			tileStringPrint(name, 400, 400, 2);
		} else if (!hasIP) {
			tileStringPrint(">", 370, 400, 1);
			tileStringPrint(IP, 400, 400, 2);
		...
		if (client.time > 120) {
			...
			int upto = 10;
			for (int i = 0; i < upto && i < overflow; i++) {// TODO add client
															// != me, I am no
															// enemy for myself
															// TODO display only
															// the BEST enemies
				...
				tileStringPrint(
						...
						350, i * 30 + 150, 9);
				tileStringPrint(String.valueOf(20 - (client.time - 120)), 466,
						50, 9);
		...
		for (int i = 0; i < dim; i++) {
			for (int j = 0; j < dim; j++) {
				...
				ttFont.drawString(field[i][j].x + 18, field[i][j].y + 12,
						String.valueOf(field[i][j].c).toUpperCase(),
						Color.black);
				...
				if (field[i][j].selected == true) {
					...
					// pointFont.drawString(field[i][j].x+30, field[i][j].y+20,
					// s);
					...
					case 0:
						...
						// s = " " +pointsOf(field[i][j].c);
						// pointFont.drawString(field[i][j].x+30,
						// field[i][j].y+20, s);
						break;
					case 1:
		...
		if (output != "") {
			if (!alreadyIn) {
				...
				// ttFont.drawString(350, 600, output.toUpperCase(),
				// Color.white);
				// ttFont.drawString(350, 650, "für "+ calcWordPoints(output)+
				// " Punkte.", Color.white);
				tileStringPrint(output.toUpperCase(), 350, 500, 1);
				tileStringPrint("gibt " + calcWordPoints(output) + " Punkte",
						350, 550, 1);
			}
		}
		if (alreadyIn && output == "") {
		...
		if ((120 - client.time) > 0) {
			...
			// Color tc = new
			// Color(255-255/60*(120-client.time),255/60*(120-client.time),0);
		...
		for (int i = 0; i < to; i++) {
			...
			if (calcWordPoints(history.get(pos)) > 9) {
				tileStringPrint(calcWordPoints(history.get(pos)) + "  "
						+ history.get(pos).toUpperCase(), 450 + xoffset,
						200 + i * 30, 0);
		...
		int upto = 10;
		for (int i = 0; i < upto && i < overflow; i++) {// TODO add client !=
														// me, I am no enemy for
														// myself
														// TODO display only the
														// BEST enemies
			overflow = client.clients.size();
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (c != ' ') {
				...
				case 0:
					...
					listFont.drawString(x + i * dx + 4, y, s
							.substring(i, i + 1).toUpperCase(), co);
					break;
				case 1:
					field_blank_correct.getScaledCopy(scale)
							.draw(x + i * dx, y);
					listFont.drawString(x + i * dx + 4, y, s
							.substring(i, i + 1).toUpperCase(), co);
					break;
				...
				case 9: // only the time
					...
					ttFont.drawString(x + i * 36 + 8, y - 4,
							s.substring(i, i + 1).toUpperCase(), co);
					break;
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (c != ' ') {
				...
				case 0:
					...
					field_blank_known.getScaledCopy(scale).draw(x + i * dx, y);
					listFont.drawString(x + i * dx + 4, y, s
							.substring(i, i + 1).toUpperCase(), Color.black);
					break;
				case 1:
					field_blank_correct.getScaledCopy(scale)
							.draw(x + i * dx, y);
				...
				case 9: // only the time
					field_blank_wrong.getScaledCopy(0.5f).draw(x + i * 36, y);
					ttFont.drawString(x + i * 36 + 8, y - 4,
							s.substring(i, i + 1).toUpperCase(), Color.black);
					break;
	...
	public void DummyField() {
		...
		for (int i = 0; i < dim; i++) {
			for (int j = 0; j < dim; j++) {
				field[i][j] = new Field(74 + j * (64 + 8) + xoffset, 68 + i
						* (64 + 8) + yoffset, i, j, letters[i][j]);
